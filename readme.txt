...
제가 드린 소스를 기준으로 설명드리면
src/main/resources/com/terry/xplatform/resources/mybatis/mapper 디렉토리에 있는
sample.xml 입니다..
그 파일을 열어보세요..
...
그리고 현재 열어놓은 xml 파일을 보시면 insert, update 태그들이 보이실겁니다..
이때 보셔야 할 것이 id 부분이에요..
...
query 가 있는 xml을 저는 VO로 하기 때문에 메소드의 파라미터들을 VO로 받았지만 SQL 작업을 하면 VO로 받지 않고
int 값 같은 단일 타입으로 받아야 할 때도 있습니다..
...
@PostConstruct 어노테이션을 통해 SampleServiceImpl이 초기화될때
sampleDAO 변수를 sqlSession.getMapper 메소드를 통해 설정해주는거에요..
...
일전에 저랑 스터디 카페에서 이 부분에 대해 얘기할때 일일이 mybatis config 파일에 등록한다고 했는데..
좀더 찾아보니 스캔해서 등록해주는 기능이 있더군요..

/src/main/resources/com/terry/xplatform/resources/mybatis/config 에 가시면
mybatis-config.xml 파일이 있습니다..
...
그러면 위에서 <package> 태그에 설정되어 있는 패키지 안의 클래스들에서 @Alias 어노테이션이 있는 것만
Mybatis의 VO로 등록해서 사용하게 됩니다..
